...
with open("card_name.txt",mode='r') as f:
  card_names = f.read().split("\n")
def data_format(data:dict):
  ...
  return data['Number'],card_names[data['Number']],h,w,data['Square'].count('Fill')+data['Square'].count('Special'),card_format,data['SpecialCost']
...
with open("card_database.hpp",mode='w') as f:
  ...
  #リストの[]を{}に変更し、空白を無くす
  for id,card_name,h,w,N_square,card_cell,SP_cost in cards_data_format:
    print(str(card_cell).replace('[','{').replace(']','}').replace(' ','')+",",file=f)
  print('};',file=f)
  print('const Card cards[N_card+1] = {',file=f)
  for i,(id,card_name,h,w,N_square,card_cell,SP_cost) in enumerate(cards_data_format):
    print(f'{"{"}u"{card_name}",{h},{w},{N_square},cards_cell[{i}],{SP_cost}{"}"},',file=f)
  print('};',file=f)
