...
template<class stage> float testplay2(
 ...
 bool logging = false
  ...
  std::vector<AI_ISMCTS<stage>> agent_P1s(num_games,AI_ISMCTS<stage>(simple_ISMCTS_num_simulations,diff_bonus,get_action_greedy<stage>,logging));
  ...
  for(int i=0;i<num_games;i++){
    agent_P1s[i].set_deck_P1(deck_P1s[i]);
    agent_P1s[i].set_deck_P2(deck_P2s[i]);
  ...
  for(int i=0;i<num_games;i++){
    if(agent_P1s[i].redraw(deck_P1s[i])) deck_P1s[i].reset();
  ...
  for(int current_turn = 1;current_turn <= Board<stage>::TURN_MAX;current_turn++){
    std::vector<Choice<stage>> choice_P1s(num_games,{-1,-1,false});
    std::vector<Choice<stage>> choice_P2s(num_games,{-1,-1,false});
    #pragma omp parallel for
    for(int i=0;i<num_games;i++){
      choice_P1s[i] = agent_P1s[i].get_action(board_P1s[i],board_P2s[i],deck_P1s[i]);
