...
public:
  ...
  Card(const std::u16string name,const int h,const int w,const int n_square,const short r0[MAX_H][MAX_W],const int sp_cost);
  //特別なカード(SP_COST==3,N_SQUARE==13,SPマスなし)
  constexpr bool is_special_card() const {return R0_SP_H == -1;};
};
Card::Card(const std::u16string name,const int h,const int w,const int n_square,const short r0[MAX_H][MAX_W],const int sp_cost):NAME(name),H(h),W(w),N_SQUARE(n_square),SP_COST(sp_cost),
