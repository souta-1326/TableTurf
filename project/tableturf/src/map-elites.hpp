...
class MAP_Elites{
  ...
  int get_feature1_index(const Feature1 &feature);
  int get_feature2_index(const Feature2 &feature);

  void add(const Individual &individual);//個体をマップに加える(弾かれることもある)
...
template<class Individual,class Feature1,class Feature2,class Fitness> int MAP_Elites<Individual,Feature1,Feature2,Fitness>::get_feature1_index(const Feature1 &feature){
  return lower_bound(boundaries1.begin(),boundaries1.end(),feature)-boundaries1.begin();
}
template<class Individual,class Feature1,class Feature2,class Fitness> int MAP_Elites<Individual,Feature1,Feature2,Fitness>::get_feature2_index(const Feature2 &feature){
  return lower_bound(boundaries2.begin(),boundaries2.end(),feature)-boundaries2.begin();
}
template<class Individual,class Feature1,class Feature2,class Fitness> void MAP_Elites<Individual,Feature1,Feature2,Fitness>::add(const Individual &individual){
  auto [feature1,feature2,fitness] = get_features_and_fitness(individual);
  ...
  if(MAP[feature1_index][feature2_index] == std::nullopt || fitness > std::get<Fitness>(MAP[feature1_index][feature2_index])){
...
template<class Individual,class Feature1,class Feature2,class Fitness> void MAP_Elites<Individual,Feature1,Feature2,Fitness>::remap(){
  ...
  for(int i=0;i<all_individuals.size();i++) features[i] = std::get<Feature1>(all_individuals[i]);
  sort(features.begin(),features.end());

  boundaries1.assign(current_size-1);
  ...
  for(int i=0;i<all_individuals.size();i++) features[i] = std::get<Feature2>(all_individuals[i]);
  sort(features.begin(),features.end());

  boundaries2.assign(current_size-1);
  //current_size-1個の境界でcurrent_size個のセルに分割する
  for(int i=0;i<current_size-1;i++){
    int sample_index = (i+1)*all_individuals.size()/current_size;
  ...
  //全個体をマップに突っ込む
  for(int i=0;i<all_individuals.size();i++) add(std::get<Individual>(all_individuals[i]));
}

template<class Individual,class Feature1,class Feature2,class Fitness> Individual MAP_Elites<Individual,Feature1,Feature2,Fitness>::generate_new_individual(){
  //G個体まではランダムに生成
  if(all_individuals.size() < G) return get_random_individual();
  //それ以降は、MAPからランダムに個体をとって突然変異させる
  else return mutate(MAP[xorshift64()%current_size][xorshift64()%current_size]);
}
...
template<class Individual,class Feature1,class Feature2,class Fitness> void MAP_Elites<Individual,Feature1,Feature2,Fitness>::step(){
  Individual new_individual = get_random_individual();

  all_individuals.emplace_back(new_individual);
  assert(all_individuals.size() <= N);
  
  if(all_individuals.size() % remap_frequency == 1) remap();
  else add(new_individual);
}